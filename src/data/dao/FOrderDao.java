...
public class FOrderDao {
	...
	public FOrderDto getData(String orderid)
	{
		FOrderDto dto = new FOrderDto();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql="select * from forder where forderid=?";
		...
		try {
			...
			if(rs.next())
			{
				...
				dto.setForderdate(rs.getTimestamp("setforderdate"));
			}
	...
	public int getTotalCount() 
	{
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql="select count(*) from forder";
		conn=db.getMyConnection();
		try {
			pstmt = conn.prepareStatement(sql);
			rs=pstmt.executeQuery();
	...
	public List<FOrderDto> getOrderList(int start, int end)
	{
		String sql="select a.* from (select ROWNUM as RNUM,b.* from "
				+ "(select * from forder order by fordernum desc)b)a where a.RNUM>=? and a.RNUM<=?";
